...
export default function NewFuelEntryScreen() {
  ...
  const distanceKm = useMemo(() => (distance.trim() ? parseIntegerInput(distance) : null), [distance]);
      ...
      <SafeAreaView edges={['bottom']} style={styles.flex}>
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          <SurfaceCard style={[styles.infoCard, { backgroundColor: theme.colors.primarySoft }]}> 
            ...
            <Text style={[styles.infoText, { color: theme.colors.text }]}>Die App berechnet Preis pro Liter und Verbrauch automatisch, sobald Strecke und Liter vorhanden sind.</Text>
          </SurfaceCard>

          <DateField label="Datum" onChange={setOccurredOn} onOpenChange={setDatePickerOpen} open={datePickerOpen} value={occurredOn} />
          <FormField inputMode="numeric" keyboardType="number-pad" label="Kilometerstand" onChangeText={setOdometer} placeholder="z. B. 137200" value={odometer} />
          <FormField inputMode="numeric" keyboardType="number-pad" label="Gefahrene Kilometer" onChangeText={setDistance} placeholder="Optional, z. B. 819" value={distance} />
          <FormField inputMode="decimal" keyboardType="decimal-pad" label="Getankte Liter" onChangeText={setLiters} placeholder="z. B. 42,51" value={liters} />
          <FormField inputMode="decimal" keyboardType="decimal-pad" label="Betrag" onChangeText={setAmount} placeholder="z. B. 65,42" value={amount} />

          {amountCents !== null && litersValue ? (
            <SurfaceCard style={styles.preview}>
              <Text style={[styles.previewText, { color: theme.colors.text }]}>Preis: {formatEuro(Math.round(amountCents / litersValue))} pro Liter</Text>
              <Text style={[styles.previewText, { color: theme.colors.text }]}>Verbrauch: {formatConsumption(previewConsumption)}</Text>
            </SurfaceCard>
          ) : null}

          <FormField label="Tankstelle" onChangeText={setStation} placeholder="Optional" value={station} />
          <Pressable
            accessibilityRole="button"
            onPress={() => setFullTank((current) => !current)}
            style={[styles.toggle, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}
          >
            <Text style={[styles.toggleText, { color: theme.colors.text }]}>{fullTank ? 'Vollgetankt' : 'Teilbetankung'}</Text>
          </Pressable>
...
const styles = StyleSheet.create({
  ...
  toggle: { minHeight: 50, borderWidth: 1, borderRadius: 15, justifyContent: 'center', paddingHorizontal: 14 },
